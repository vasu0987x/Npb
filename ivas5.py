...
# ── SPEED TUNING ──────────────────────────────────────────────
OTP_POLL_INTERVAL   = 0.5      # Seconds between poll cycles (was 1)
CMD_POLL_INTERVAL   = 0.3      # Fast command response
NUM_SMS_THREADS     = 20       # Parallel SMS fetch threads (shared global pool)
NUM_MSG_SENDERS     = 3        # Parallel Telegram message senders
CSRF_REFRESH_EVERY  = 100      # Refresh CSRF token every N iterations
HTTP_TIMEOUT        = 5        # HTTP request timeout - fail fast (was 6)
TG_API_TIMEOUT      = 5        # Telegram API timeout
TG_RATE_LIMIT_DELAY = 0.05     # 50ms between sends = ~20 msgs/sec safe
# ─────────────────────────────────────────────────────────────
...
# ONE shared pool — prevents the "N futures unfinished" explosion.
# All 274 numbers share these 15 threads; no pile-up possible.
...
def create_session():
    s = requests.Session()
    # Use connection pooling with higher limits for speed
...
_tg_lock = threading.Lock()   # only for LAST_UPDATE_ID, not for requests
...
def _admin_cb_seensms(chat_id, message_id, user_id):
    ...
    msg = (
        ...
        f"🔀 OTP Workers: <b>1 (shared pool)</b>\n"
...
def process_number(csrf, num, rname, session):
    ...
    try:
        ...
        print(f"⚠️ process_number error {num}: {e}")

# ╔══════════════════════════════════════════════════════════════╗
# ║        WORKER 1 — COMMAND PROCESSOR (parallel dispatch)     ║
# ╚══════════════════════════════════════════════════════════════╝
_cmd_executor = ThreadPoolExecutor(max_workers=6, thread_name_prefix="CmdHandler")

def dispatch_update(update):
    """Handle a single update in its own thread — zero blocking."""
    try:
...
# ╔══════════════════════════════════════════════════════════════╗
# ║        WORKER 2 — OTP MONITOR (one instance per session)    ║
# ╚══════════════════════════════════════════════════════════════╝
# ╔══════════════════════════════════════════════════════════════╗
# ║        WORKER 2 — OTP MONITOR (ultra-fast cached)           ║
# ╚══════════════════════════════════════════════════════════════╝

# Cache: number → range_name  (refreshed every N seconds, not every cycle)
_NUM_RANGE_CACHE      = {}   # {number: range_name}
_NUM_RANGE_CACHE_TIME = 0
_NUM_RANGE_CACHE_TTL  = 30   # seconds — refresh number list every 30s
_NUM_RANGE_LOCK       = threading.Lock()

def _refresh_num_range_cache(csrf, session):
    """Rebuild number→range map. Called once every 30s, not every cycle."""
    global _NUM_RANGE_CACHE, _NUM_RANGE_CACHE_TIME
    try:
        ...
        def fetch_one_range(rname):
            try:
                numbers = get_numbers_from_range(csrf, rname, session)
                with pairs_lock:
                    for num in numbers:
                        new_cache[str(num)] = rname
            except Exception as e:
                print(f"⚠️ Range fetch error {rname}: {e}")

        # All ranges in parallel
        futs = [SMS_EXECUTOR.submit(fetch_one_range, rname) for rname in ranges]
        for f in futs:
            try:
                f.result(timeout=HTTP_TIMEOUT)
            except Exception:
                pass

        with _NUM_RANGE_LOCK:
            _NUM_RANGE_CACHE      = new_cache
            _NUM_RANGE_CACHE_TIME = time.time()

        print(f"🗂️ Num→Range cache refreshed: {len(new_cache)} numbers across {len(ranges)} ranges")
        return True
    except Exception as e:
...
def otp_worker(worker_id=0):
    """
    ULTRA-FAST design:
    - Number→Range map cached for 30s  →  skip trigger_getsms + get_numbers_from_range every cycle
    - Each cycle ONLY does: get_sms per number (pure parallel, 20 threads)
    - No blocking wait on futures — fire and forget, track in_flight passively
    - Priority: assigned-user numbers polled first → instant DM delivery
    """
    print(f"📱 OTP Worker-{worker_id} Started")
    session = create_session()
    csrf    = None
    ...
    _refresh_num_range_cache(csrf, session)
    ...
    while BOT_RUNNING:
        try:
            ...
            # ── Refresh number→range cache every 30s ─────────────
            with _NUM_RANGE_LOCK:
                cache_age  = time.time() - _NUM_RANGE_CACHE_TIME
                cache_copy = dict(_NUM_RANGE_CACHE)

            if cache_age > _NUM_RANGE_CACHE_TTL:
                # Refresh in background — don't block current cycle
                threading.Thread(
                    target=_refresh_num_range_cache,
                    args=(csrf, session),
                    daemon=True
                ).start()

            if not cache_copy:
                # No cache yet — wait for it
                time.sleep(1)
                continue
            ...
            # ── Split into priority (assigned user) vs secondary ──
            user_db          = load_user_db()
            assigned_numbers = {str(v["number"]) for v in user_db.values()}

            priority_pairs  = []
            secondary_pairs = []
            for num, rname in cache_copy.items():
                if num in assigned_numbers:
                    priority_pairs.append((num, rname))
                else:
                    secondary_pairs.append((num, rname))

            # ── Fire SMS fetches — priority first ─────────────────
            for num, rname in priority_pairs:
                f = SMS_EXECUTOR.submit(process_number, csrf, num, rname, session)
                in_flight.append(f)

            for num, rname in secondary_pairs:
                f = SMS_EXECUTOR.submit(process_number, csrf, num, rname, session)
                in_flight.append(f)

            # ── Tiny sleep to avoid hammering portal ─────────────
...
# ╔══════════════════════════════════════════════════════════════╗
# ║        WORKER 3 — MESSAGE SENDERS (multiple parallel)        ║
# ╚══════════════════════════════════════════════════════════════╝
def message_sender_worker(sender_id=0):
    """
    Drains MESSAGE_QUEUE fast but respects Telegram rate limits.
    - DM messages (user) are sent with NO delay (instant personal delivery)
    - Group messages get TG_RATE_LIMIT_DELAY between them (2-5/sec safe)
    - Multiple senders run in parallel but each handles their own items
    """
    ...
    while BOT_RUNNING:
        try:
            ...
            # Rate limiting: group gets slight delay, DM is instant
            if msg_type == "group":
...
def main():
    ...
    print("=" * 60)
    print()
    print(f"⚡ SPEED CONFIG:")
    print(f"   OTP Poll Interval : {OTP_POLL_INTERVAL}s")
    print(f"   CMD Poll Interval : {CMD_POLL_INTERVAL}s")
    print(f"   SMS Threads       : {NUM_SMS_THREADS} (shared pool)")
    print(f"   Msg Senders       : {NUM_MSG_SENDERS}")
    print(f"   TG Rate Limit     : {TG_RATE_LIMIT_DELAY*1000:.0f}ms/group msg")
    print()
    ...
    # Single OTP worker (shared pool handles parallelism internally)
    workers.append(threading.Thread(target=otp_worker, args=(0,), name="OTPWorker", daemon=True))

    # Multiple message senders
