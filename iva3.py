...
# ── SPEED TUNING ──────────────────────────────────────────────
OTP_POLL_INTERVAL   = 1        # Was 2 — faster OTP polling
CMD_POLL_INTERVAL   = 0.3      # Was 1 — faster command response
NUM_OTP_WORKERS     = 3        # Parallel OTP polling sessions
NUM_SMS_THREADS     = 10       # Parallel SMS fetch threads per worker
NUM_MSG_SENDERS     = 3        # Parallel Telegram message senders
CSRF_REFRESH_EVERY  = 80       # Refresh CSRF less often (was 50)
HTTP_TIMEOUT        = 8        # Was implicit; explicit for speed
TG_API_TIMEOUT      = 6        # Telegram API timeout
# ─────────────────────────────────────────────────────────────
...
# Shared thread pool for SMS fetching
SMS_EXECUTOR = ThreadPoolExecutor(max_workers=NUM_SMS_THREADS)
...
def _admin_cb_seensms(chat_id, message_id, user_id):
    ...
    msg = (
        ...
        f"🔀 OTP Workers: <b>{NUM_OTP_WORKERS}</b>\n"
...
def otp_worker(worker_id=0):
    """
    Each otp_worker gets its OWN session + CSRF.
    They all poll the same ranges but process different numbers
    in parallel via an internal thread pool.
    """
    ...
    for attempt in range(1, 4):
        try:
            print(f"🔑 Worker-{worker_id} login attempt {attempt}/3...")
            csrf = login(session)
            csrf = get_csrf_for_sms(session)
            print(f"✅ OTP Worker-{worker_id} logged in")
            break
        except Exception as e:
            print(f"❌ Worker-{worker_id} login failed ({attempt}): {e}")
            if attempt < 3:
                time.sleep(5)
            else:
                print(f"❌ Worker-{worker_id} max attempts reached. Stopping.")
                return

    csrf_counter = 0
    iteration    = 0

    # Each worker uses its own small thread pool for SMS fetching
    worker_sms_pool = ThreadPoolExecutor(
        max_workers=NUM_SMS_THREADS,
        thread_name_prefix=f"SMS-W{worker_id}"
    )

    # Stagger workers so they don't all hit at the exact same second
    time.sleep(worker_id * (OTP_POLL_INTERVAL / NUM_OTP_WORKERS))

    while BOT_RUNNING:
        try:
            iteration += 1
            if iteration % 30 == 0:
                print(f"💓 OTP Worker-{worker_id} heartbeat — iter {iteration}")
            ...
            if csrf_counter >= CSRF_REFRESH_EVERY:
                try:
                    ...
                    try:
                        ...
                        print(f"❌ Worker-{worker_id} re-auth failed: {e}")
                        time.sleep(10)
                        continue
            csrf_counter += 1
            ...
            # Gather all (range, number) pairs first
            all_pairs = []
            for rname in ranges:
                try:
                    numbers = get_numbers_from_range(csrf, rname, session)
                    for num in numbers:
                        all_pairs.append((rname, num))
                except Exception as e:
                    print(f"⚠️ Worker-{worker_id} range error {rname}: {e}")

            # Fetch SMS for all numbers in parallel
            futures = [
                worker_sms_pool.submit(process_number, csrf, num, rname, session)
                for rname, num in all_pairs
            ]
            ...
            for future in as_completed(futures, timeout=30):
                try:
                    ...
                    print(f"⚠️ Worker-{worker_id} future error: {e}")

            time.sleep(OTP_POLL_INTERVAL)

        except Exception as e:
            print(f"⚠️ OTP Worker-{worker_id} Error: {e}")
            time.sleep(5)
...
def message_sender_worker(sender_id=0):
    """Multiple sender workers drain the queue in parallel."""
...
def main():
    ...
    print(f"   OTP Workers       : {NUM_OTP_WORKERS}")
    print(f"   SMS Threads/Worker: {NUM_SMS_THREADS}")
    ...
    # Command worker (single — fast polling handles it)
    workers.append(threading.Thread(target=command_worker, name="CommandWorker", daemon=True))

    # Multiple OTP workers
    for i in range(NUM_OTP_WORKERS):
        workers.append(threading.Thread(
            target=otp_worker, args=(i,),
            name=f"OTPWorker-{i}", daemon=True
        ))
