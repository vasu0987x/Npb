...
BASE_URL = "https://app.thirdwave.im/api/v1/traffic"
PANEL_NAME = "thirdwave"
API_PER_PAGE = 100
...
def parse_row(row):
    try:
        ...
        if row.get("status", "").upper() != "DELIVERED":
            return None

        sender = str(row.get("sourceAddress", "") or "").strip()
        number = str(row.get("destinationNumber", "") or "").strip().lstrip("+").lstrip("0")

        message = str(row.get("messageBody", "") or "").replace("\\n", "\n").replace("\\\\n", "\n")
        message = html.unescape(message).strip()

        dt = str(row.get("receivedAt", "") or "").strip()
...
def fetch_thirdwave_page(page: int = 1, timeout: int = 10, retry_on_429: bool = False, attempt: int = 1, number: str = None, from_date: str = None):
    try:
        headers = {
        ...
        if resp.status_code == 429:
            try:
                ...
                retry_after = int(numbers_found[0]) if numbers_found else 5
            except Exception:
                retry_after = 5
                
            print(f"⏳ {PANEL_NAME}: 429 Rate limit — {retry_after}s wait (attempt {attempt})", flush=True)
            if retry_on_429 and attempt < 3:
                time.sleep(retry_after + 1)
                return fetch_thirdwave_page(page, timeout, retry_on_429, attempt + 1, number, from_date)
            else:
                return None
                
        if resp.status_code == 401:
            print(f"❌ {PANEL_NAME}: API key invalid ya expired (401)", flush=True)
            return None
            
        if resp.status_code != 200:
            print(f"⚠️ {PANEL_NAME}: HTTP {resp.status_code} - {resp.text}", flush=True)
            return None
            
        payload = resp.json()
        if not isinstance(payload, dict):
            print(f"⚠️ {PANEL_NAME}: Invalid response format", flush=True)
            return None
            
        if "error" in payload:
            print(f"⚠️ {PANEL_NAME}: {payload.get('error')} - {payload.get('message','')}", flush=True)
            return None
            
        return payload
    except Exception as e:
        print(f"❌ {PANEL_NAME} fetch error (page {page}): {e}", flush=True)
        return None
OTP_GROUP_IDS = ["-5350249682"]
...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            ...
            OTP_GROUP_IDS = data.get("otp_groups", ["-5350249682"])
            AUTO_DELETE_MINUTES = data.get("auto_delete_minutes", 0)
    else:
        data = {"numbers_by_country": {}, "current_country": None, "otp_groups": ["-5350249682"], "auto_delete_minutes": 0}
        numbers_by_country = {}
        current_country = None
...
def otp_scraper_thread():
    ...
    while True:
        try:
            payload = fetch_thirdwave_page(page=1, timeout=15, retry_on_429=True)
            
            if payload is None:
                print("⚠️ Scraper: Failed to fetch page 1 from API, retrying in 10s...", flush=True)
                time.sleep(10)
                continue
            
            rows = payload.get("rows", [])
            if not rows:
                print("ℹ️ Scraper: No rows returned from API.", flush=True)
                time.sleep(10)
                continue
            ...
            for row in reversed(rows):
                ...
                try:
                    ...
                    print(f"⚠️ Group queue full: {e}!", flush=True)
                ...
                if chat_id:
                    try:
                        ...
                        print(f"⚠️ Personal queue full for {chat_id}: {e}!", flush=True)
...
def fetch_past_otps(chat_id, number):
    ...
    try:
        ...
        # Query Thirdwave API specifically for the target number
        payload = fetch_thirdwave_page(page=1, timeout=8, retry_on_429=False, number=number)
