...
# ║                   INLINE BUTTON LINKS                        ║
# ╚══════════════════════════════════════════════════════════════╝
PANEL_LINK     = "https://t.me/DDXOTPBOT"       # ← Bot/Panel ka link
CHANNEL_LINK   = "https://t.me/ddxotp"          # ← Channel ka link
OTP_GROUP_LINK = "https://t.me/+SDPuI2Ud62RkN2Jl"  # ← OTP Group ka invite link
...
MESSAGE_QUEUE = Queue()
SMS_TEXT_CACHE = {}   # { hash_id: sms_text } — fullsms popup ke liye
...
FLAG_OVERRIDES      = _settings.get("flag_overrides", {})   # { "IN": "emoji_id", ... }
...
# ╔══════════════════════════════════════════════════════════════╗
# ║                 SERVICE CODES  (sender → 2-letter code)      ║
# ╚══════════════════════════════════════════════════════════════╝
SERVICE_CODES = {
...
def send_force_subscribe_message(chat_id, not_joined_channels):
    ...
    "url": channel["link"],
    "icon_custom_emoji_id": "6219641556945606133"  # This is the ID for the Green Checkmark emoji
}])
    msg += "\n━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    "icon_custom_emoji_id": "6255900302218628064"  # Example ID
}])
...
def get_country_flag(country_name):
    ...
    # Check FLAG_OVERRIDES (premium tg-emoji)
...
def mask_number(number):
    """Same format as dokivasms: first2 + DDX + last4"""
    if len(number) <= 4:
        return number
...
# ╔══════════════════════════════════════════════════════════════╗
# ║               MESSAGE FORMATTER  (dokivasms style)           ║
# ╚══════════════════════════════════════════════════════════════╝
def format_otp_message_group(range_name, number, sms_text, otp, sender=""):
    """
    Group mein compact format — bilkul dokivasms jaisa:
    ⚡ 🇮🇳 IN | 91DDX1234 | 🌐 WA
    """
    country     = extract_country_name(range_name)
...
def format_otp_message_personal(range_name, number, sms_text, otp, sender=""):
    """
    Personal DM mein full detail format — bilkul dokivasms jaisa
    """
    country    = extract_country_name(range_name)
    flag       = get_country_flag(country)
...
def _admin_cb_broadcast_help(chat_id, message_id, user_id):
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━\n"
        "<b>Example:</b>\n"
        "<code>/broadcast Bot maintenance\n"
        "ke liye 1 hour ke liye band rahega.</code>"
    )
    edit_telegram_message(chat_id, message_id, msg, {"inline_keyboard": [_back_btn()]})
...
def handle_autodelete_command(chat_id, user_id, message_text):
    """
    /autodelete <minutes>   — group messages auto-delete timer set karo
    /autodelete 0           — disable karo
    /autodelete             — current status dekho
    """
...
def handle_addflag_command(chat_id, user_id, message_text):
    """
    /addflag IN 5222300011366200403
    Country code (2-letter) + premium tg-emoji ID
    """
    if user_id not in ADMIN_USER_IDS:
        send_telegram_message(chat_id, "❌ Sirf admins ke liye.")
        return
    ...
    if len(parts) != 3:
        send_telegram_message(chat_id,
            ...
            "IN = 2-letter country code (e.g. IN for India, PK for Pakistan)\n"
...
def handle_removeflag_command(chat_id, user_id, message_text):
    ...
    if len(parts) != 2:
        send_telegram_message(chat_id,
            "❌ <b>Usage:</b> <code>/removeflag IN</code>")
        return
    code = parts[1].upper()
    if code in FLAG_OVERRIDES:
...
def get_sms(csrf, number, range_name, session):
    """
    Returns list of (sender, sms_text) tuples.
    sender comes from <span class="cli-tag">WhatsApp</span> in the same row.
    ...
    try:
        ...
        # Try to pair cli-tag + msg-text from same table row
        # HTML structure: <tr> contains <td><span class="cli-tag">...</span></td> and <td><div class="msg-text">...</div></td>
        ...
        # Fallback: just grab msg-text divs if row-based parse gave nothing
        if not results:
            for div in soup.find_all("div", class_="msg-text"):
                text = div.get_text(strip=True)
                if text and len(text) > 3:
                    # Try to find nearest cli-tag sibling in parent td/tr
                            ...
                            for sender, sms in sms_list:
                                with SMS_LOCK:
                                    ...
                                    if h in SEEN_SMS:
                                        continue
                                    SEEN_SMS.add(h)
                                    SMS_TEXT_CACHE[h] = sms   # fullsms ke liye store karo
    ...
    while BOT_RUNNING:
        try:
            if not MESSAGE_QUEUE.empty():
                ...
                # Build inline keyboard (dokivasms style with premium emoji icons)
                keyboard = None
                if msg_type == "group":
                    buttons = []
                    if otp:
                        buttons.append([{
                            ...
                            "icon_custom_emoji_id": "5258500400918587241"   # 🔑 key
                        }])
                    if hash_id:
                        buttons.append([{
                            "text": "View Full SMS",
                            "callback_data": f"fullsms_{hash_id}",
                            "icon_custom_emoji_id": "5257980374868311346"   # 👁 eye
                        }])
                    buttons.append([
                        {
                            "text": "Panel",
                            "url": PANEL_LINK,
                            "icon_custom_emoji_id": "5145427681680032825"   # 🤖 robot
                        },
                        {
                            "text": "Channel",
                            "url": CHANNEL_LINK,
                            "icon_custom_emoji_id": "6219641556945606133"   # 📢 channel
                        }
                    ])
                ...
                # Auto-delete for group messages
...
def main():
    print("=" * 60)
    print("🤖  PROFESSIONAL OTP BOT  |  Multi-Threaded")
    print("=" * 60)
    print()
