...
# ── SPEED TUNING ──────────────────────────────────────────────
OTP_POLL_INTERVAL   = 1        # Seconds between poll cycles
CMD_POLL_INTERVAL   = 0.3      # Fast command response
NUM_SMS_THREADS     = 15       # Parallel SMS fetch threads (shared global pool)
NUM_MSG_SENDERS     = 3        # Parallel Telegram message senders
CSRF_REFRESH_EVERY  = 100      # Refresh CSRF token every N iterations
HTTP_TIMEOUT        = 6        # HTTP request timeout (fail fast)
...
# ╚══════════════════════════════════════════════════════════════╝
# ╔══════════════════════════════════════════════════════════════╗
# ║        WORKER 2 — OTP MONITOR (smart, single session)       ║
# ╚══════════════════════════════════════════════════════════════╝
def otp_worker(worker_id=0):
    """
    KEY OPTIMIZATIONS vs old version:
    1. Uses ONE shared SMS_EXECUTOR — no per-worker pools that pile up
    2. Submits futures and moves on immediately — doesn't wait with as_completed
       (waiting caused "N futures unfinished" timeout errors)
    3. Only polls numbers that have assigned users FIRST (instant delivery)
       then polls remaining numbers for the group channel
    4. Uses a simple in-flight counter to avoid over-submitting
    """
    ...
    csrf_counter = 0
    iteration    = 0
    # Track in-flight futures to avoid unbounded queue growth
    in_flight    = []
    ...
    while BOT_RUNNING:
        try:
            ...
            # ── Clean up finished futures (no blocking wait) ──────
            in_flight = [f for f in in_flight if not f.done()]

            # ── Throttle: if pool is still busy, skip this cycle ──
            ...
            # ── Build number list ─────────────────────────────────
            # Priority: assigned numbers first (instant DM + group)
            # then unassigned numbers (group only)
            user_db = load_user_db()
            assigned_numbers = {str(v["number"]): v for v in user_db.values()}

            priority_pairs   = []   # has an assigned user
            secondary_pairs  = []   # no assigned user

            for rname in ranges:
                try:
                    numbers = get_numbers_from_range(csrf, rname, session)
                    for num in numbers:
                        if str(num) in assigned_numbers:
                            priority_pairs.append((rname, num))
                        else:
                            secondary_pairs.append((rname, num))
                except Exception as e:
                    print(f"⚠️ Range error {rname}: {e}")

            # Submit priority numbers immediately
            for rname, num in priority_pairs:
                f = SMS_EXECUTOR.submit(process_number, csrf, num, rname, session)
                in_flight.append(f)

            # Submit secondary numbers (group-only) right after
            for rname, num in secondary_pairs:
                f = SMS_EXECUTOR.submit(process_number, csrf, num, rname, session)
                in_flight.append(f)

            if priority_pairs or secondary_pairs:
                total = len(priority_pairs) + len(secondary_pairs)
                print(f"📡 Submitted {total} numbers "
                      f"({len(priority_pairs)} priority, {len(secondary_pairs)} secondary)")
