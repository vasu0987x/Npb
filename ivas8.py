...
EXTRA_CHATS = load_extra_chats()  # list of chat_ids
...
def get_telegram_updates():
    ...
    # timeout=2 = 2 second long poll, instant response milega
    data = _tg_get("getUpdates", {"offset": offset, "timeout": 2, "limit": 100})
...
def handle_start_command(chat_id, user_id, username):
    # Pehle ek instant "loading" message bhejo
...
def message_sender_worker(sender_id=0):
    ...
    while BOT_RUNNING:
        try:
            ...
            # REPLACE: result = send_telegram_message(chat_id, message, keyboard)
             # WITH:
            ...
            if msg_type == "group":
              # Main group
                 result = send_telegram_message(chat_id, message, keyboard)
                  # Extra chats
                 for extra_cid in EXTRA_CHATS:
                   try:
                      send_telegram_message(extra_cid, message, keyboard)
                   except Exception as e:
                     print(f"⚠️ Extra chat {extra_cid} error: {e}")
            else:
              result = send_telegram_message(chat_id, message, keyboard)
